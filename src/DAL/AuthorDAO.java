...
//--delete--    
    public boolean deleteByID(int id) {
        int rs = executeUpdate("UPDATE AUTHOR SET existing_logic=false WHERE author_id=" + id);
        if (rs > 0) return true;
        return false;
    }

    public boolean deleteByName(String name) {
        int rs = executeUpdate("UPDATE AUTHOR SET existing_logic=false WHERE author_name='"+name+"'");
        if (rs > 0) return true;
        return false;
    }
